...
CREATE SEQUENCE SEQ_BOARD_NO; -- 회원 번호 시퀀스
CREATE SEQUENCE SEQ_REPLY_NO; -- 회원 번호 시퀀스
...
-- 회원 테이블
INSERT INTO MEMBER 
VALUES( SEQ_MEMBER_NO.NEXTVAL, 'user01', 'pass01!', '홍길동', 
        '010-1234-5678', 'user01@kh.or.kr', '서울시 중구 남대문로 120 대일빌딩 2층',
        DEFAULT, 100, 200);
...
-- 정지 회원 샘플 데이터
INSERT INTO MEMBER 
VALUES( SEQ_MEMBER_NO.NEXTVAL, 'user02', 'pass02!', '고길동', 
        '010-1234-5678', 'user01@kh.or.kr', '서울시 중구 남대문로 120 대일빌딩 2층',
        DEFAULT, 102, 200);
...
-- STATUS_CD 기본값 지정
ALTER TABLE MEMBER
MODIFY STATUS_CD DEFAULT 100;

-- GRADE_CD 기본값 지정
ALTER TABLE MEMBER
MODIFY GRADE_CD DEFAULT 200;
...
BEGIN
    FOR I IN 1..500 LOOP
        ...
        VALUES(
            ...
            24, 300, FLOOR(DBMS_RANDOM.VALUE(1,4))
            );
...
BEGIN
    FOR I IN 1..100 LOOP
        ...
        VALUES(
            ...
            SEQ_BOARD_NO.CURRVAL || '번째 게시글입니다.',
            DEFAULT, DEFAULT, DEFAULT, 
            24, 300 + FLOOR(DBMS_RANDOM.VALUE(0,3)), FLOOR(DBMS_RANDOM.VALUE(1,4))
            );
...
-- 게시글 목록 조회
-- 글번호, 제목, 작성자, 조회수, 작성일, 카테고리, 상태
    
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NO, MEMBER_NM, READ_COUNT, 
       
       -- CREATE_DT,
       -- 게시글 작성 시간으로 부터 1일이 지나지 않은 경우 -> 16:10 시간 작성
       -- 1일이 지난 경우 -> 2021-12-13 날짜 작성
...
-- 서브쿼리를 통해 정렬 후 ROWNUM 적용
-- 서브쿼리에 별칭을 주고 별칭.*을 작성하면 다른 컬럼과 함께 사용할 수 있다.
-- 가상컬럼 ROWNUM에 별칭을 주면 실질적인 컬럼으로 변한다.
-- WHERE 절에도 ROWNUM이 아닌 RNUM으로 조회하면 알맞은 결과가 나온다.
...
COMMENT ON COLUMN "BOARD_IMG"."IMG_LEVEL" IS '이미지 위치를 지정값 저장';
...
-- BOARD 테이블 BOARD_STATUS_CD 컬럼 기본값 지정하기
ALTER TABLE BOARD 
MODIFY BOARD_STATUS_CD DEFAULT 300;
...
INSERT INTO REPLY_STATUS VALUES(1, '정상');
INSERT INTO REPLY_STATUS VALUES(2, '블라인드');
INSERT INTO REPLY_STATUS VALUES(3, '삭제');
COMMIT;

-- BOARD 테이블 BOARD_STATUS_CD 컬럼 기본값 지정
ALTER TABLE REPLY MODIFY REPLY_STATUS_CD DEFAULT 1;
...
VALUES(SEQ_REPLY_NO.NEXTVAL, '1번 테스트 댓글입니다.', 611, 24);


INSERT INTO REPLY(REPLY_NO, REPLY_CONTENT, BOARD_NO, MEMBER_NO)
VALUES(SEQ_REPLY_NO.NEXTVAL, '2번 테스트 댓글입니다.', 611, 24);


INSERT INTO REPLY(REPLY_NO, REPLY_CONTENT, BOARD_NO, MEMBER_NO)
VALUES(SEQ_REPLY_NO.NEXTVAL, '3번 테스트 댓글입니다.', 611, 24);


INSERT INTO REPLY(REPLY_NO, REPLY_CONTENT, BOARD_NO, MEMBER_NO)
VALUES(SEQ_REPLY_NO.NEXTVAL, '4번 테스트 댓글입니다.', 611, 24);


INSERT INTO REPLY(REPLY_NO, REPLY_CONTENT, BOARD_NO, MEMBER_NO)
VALUES(SEQ_REPLY_NO.NEXTVAL, '5번 테스트 댓글입니다.', 611, 24);
